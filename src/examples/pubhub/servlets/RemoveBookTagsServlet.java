...
public class RemoveBookTagsServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (tempBook == null) {
			...
			if (tempBookTag == null){
					req.getSession().setAttribute("message", "Book tag does not exist in the system");
					req.getSession().setAttribute("messageClass", "alert-danger");
					req.getRequestDispatcher("removeBookTag.jsp").forward(req, resp);
			} else {
				...
				boolean isSuccess = bookTagdatabase.deleteBookTag(tempBookTag);
				
				if(isSuccess){
					req.getSession().setAttribute("message", "Book Tag successfully removed");
